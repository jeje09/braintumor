...
1. 영수증 인쇄 쏠림 현상(선 걸침/오프셋) 엄격 보정: 
   - 영수증 원본을 볼 때 가장 첫 번째 항목(예: '진찰료')의 금액 숫자가 **해당 항목 칸 위쪽에 치우쳐서(혹은 위쪽 테두리 선에 걸쳐서) 인쇄되어 있는지 반드시 확인하세요.**
   - 만약 첫 번째 항목의 금액이 위쪽으로 치우쳐 있다면, **아래에 있는 모든 칸의 금액들 역시 똑같이 한 칸씩 위쪽으로 치우쳐 인쇄된 것입니다.** (즉, 주사료(행위료) 칸에 인쇄된 큰 금액은 사실 그 칸의 금액이 아니라 **바로 아래 칸인 주사료(약품비)**의 금액일 확률이 100%입니다.)
   - 이 '위쪽 치우침(오프셋)' 규칙을 파악하여 엉뚱한 항목(특히 비싼 약품비가 행위료로 들어가는 등)에 금액이 매칭되지 않도록 모든 금액을 한 칸씩 끌어내려 정확한 항목에 짝지어주세요.
