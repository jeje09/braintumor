...
[매우 중요한 판독 규칙]
1. OCR 오인식 보정 (선 걸침): 금액이 표의 가로선(테두리 선)에 걸쳐서 인쇄된 경우 무조건 **그 선의 아래쪽 항목**에 해당하는 금액으로 매칭하세요.
2. 빈칸(금액이 전혀 없는 항목)은 절대 JSON 결과에 포함하지 마세요. 금액이 발생한 유효 항목만 배열에 추가하세요.
3. 명칭 통일 기준 (통계를 위해 필수): 
   - 의사/약사의 행위나 서비스 대가는 "-료 (행위료)"로, 물품이나 재화 비용은 "-비 (물품비)"로 통일합니다.
   - 예시: "약품료", "약품비" -> "약품비" 로 기재. "재료대" -> "재료비" 로 기재.
   - 세부 분류가 있는 경우 반드시 괄호를 사용해 구체적으로 적으세요.
   - 예: "입원료" 하위에 "1인실"이 있다면 -> "입원료(1인실)"
   - 예: "투약 및 조제료" 하위에 "행위료", "약품비"가 있다면 -> "투약 및 조제료(행위료)", "투약 및 조제료(약품비)"
   - 예: "진찰료" -> "진찰료"
4. 그룹 구분 (group):
   - 일반적인 급여/비급여 기본 검사 및 입원료 등은 "기본항목"
   - 선택진료료, CT, MRI, PET, 초음파, 보철교정료 등 영수증 하단의 선택 항목들은 "선택항목"
   - 기타 알 수 없는 항목은 "기타항목"
...
분석을 원하는 필수 항목 스키마 (반드시 이 구조를 지킬 것):
{
  ...
  "items": [
    // 금액이 존재하는 항목들만 배열에 추가
    { 
      "group": "기본항목", 
      "name": "항목명(세부명)", 
      "본인부담금": null, // 숫자 또는 null
      "공단부담금": null, 
      "전액본인부담금": null, 
      "비급여": null 
    }
  ],
  "analysisSummary": "환자가 이해하기 쉬운 1-2문장의 영수증 분석 요약 (본인부담금과 비급여 특징 위주로)"
}
