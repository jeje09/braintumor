...
[매우 중요한 판독 규칙]
1. 영수증 인쇄 쏠림 현상(선 걸침/오프셋) 엄격 보정: 
   - 영수증 원본을 볼 때 가장 첫 번째 항목(예: '진찰료')의 금액 숫자가 **해당 항목 칸 위쪽에 치우쳐서(혹은 위쪽 테두리 선이나 표의 헤더 영역에 걸쳐서) 인쇄되어 있는지 반드시 확인하세요.**
   - 만약 금액들이 위쪽으로 치우쳐 있다면, **아래에 있는 모든 칸의 금액들 역시 똑같이 한 칸씩 위쪽으로 치우쳐 인쇄된 것입니다.** (예: 주사료(행위료) 칸에 인쇄된 큰 금액은 사실 그 아래 칸인 주사료(약품비)의 금액입니다.)
   - 따라서 엉뚱한 항목에 금액이 매칭되지 않도록 모든 금액을 한 칸씩 끌어내려 정확한 항목에 짝지어주세요.
   - **[주의!] 이 때 제일 첫 번째 항목인 '진찰료'가 누락되어서는 절대 안 됩니다. 금액들이 위로 한 칸씩 밀려 올라갔다면, '진찰료'의 진짜 금액은 그 칸의 위쪽 테두리나 항목명(헤더) 영역에 인쇄되어 있을 것입니다. 그 값을 찾아서 반드시 진찰료의 금액으로 기재하세요.**
2. 빈칸(금액이 전혀 없는 항목)은 절대 JSON 결과에 포함하지 마세요. 단, 진찰료 등 실제로 금액이 있는데 인쇄가 밀려서 다른 곳에 적힌 것은 빈칸이 아니므로 꼭 찾아서 넣어야 합니다.
