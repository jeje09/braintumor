  ...
  return (
    <div className="space-y-10 pb-16">
      ...
      <section className="space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300 text-xs font-bold">
            ...
            <button
              ...
              className={`p-4 rounded-2xl border text-left transition-all flex flex-col justify-between h-28 ${
                isSelected
                  ...
                  : 'glass-card border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-violet-300'
              }`}
            >
              <span className="text-2xl">{t.icon}</span>
              <div>
                <span className={`text-[10px] font-bold block ${isSelected ? 'text-violet-200' : 'text-slate-400'}`}>
                  {t.grade}
                </span>
                <h3 className="font-extrabold text-xs line-clamp-1">{t.name}</h3>
              </div>
            </button>
          );
        })}
      ...
      {selectedTumor && (
        <div className="glass-card rounded-3xl p-6 sm:p-10 border border-violet-100 dark:border-slate-800 space-y-8 animate-fade-in-up">
          
          {/* Header */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-3">
              <span className="text-4xl p-3 bg-violet-50 dark:bg-violet-950 rounded-2xl">{selectedTumor.icon}</span>
              <div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full text-white ${selectedTumor.colorBadge}`}>
                    {selectedTumor.grade}
                  </span>
                  <span className="text-xs text-slate-400 font-medium">{selectedTumor.fullName}</span>
                </div>
            ...
            {selectedTumor.isSpecial && (
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs transition-all flex items-center gap-1.5 shadow-md shadow-rose-600/30"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2">
              <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">예후 및 치료 반응</h4>
            ...
            {/* Key Markers */}
            <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2">
              <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">핵심 진단·유전자 인자</h4>
